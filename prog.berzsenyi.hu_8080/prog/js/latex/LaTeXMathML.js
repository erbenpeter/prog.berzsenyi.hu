...
function translate(spanclassAM) {
  if (!AMtranslated) { // run this only once
    ...
    //F.G.'s modification... parse only <span class="mathspan">...</span> areas
    var pattern = new RegExp("(^|\\s)"+"mathspan"+"(\\s|$)");
    AMlist = document.getElementsByTagName("span"); 
    for (var i = 0; i < AMlist.length; i++) { 
        AMbody = AMlist[i];

        if (pattern.test(AMbody.className) ) {
